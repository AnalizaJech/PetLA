...
<x-app-layout>
    <div class="container mx-auto px-4 py-1 bg-white rounded-lg shadow-sm border border-gray-200 mt-5">
        ...
        <div class="p-4">
            <form action="{{ route('citas.update', $cita->id)  }}" class="space-y-6" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @foreach($mascotas as $mascota)
                                    <option  value="{{ $mascota->id }}" {{ $mascota->id == $cita->mascota_id ? 'selected' : '' }}>{{ $mascota->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @foreach($veterinarios as $veterinario)
                                    <option value="{{ $veterinario->id }}" {{ $veterinario->id == $cita->veterinario_id ? 'selected' : '' }}>{{ $veterinario->name }}</option>
                                @endforeach
                            </select>
                            <i class="fas fa-chevron-down absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                        </div>
