    ...
    <div class="flex items-start justify-between">
        ...
        <div class="flex flex-col space-y-2">
            ...
            <form id="form-delete-{{ $precita->id }}" action="{{ route('precitas.destroy', $precita->id) }}" method="POST">
                ...
                <button  
                    type="button"
                    onclick="confirmDelete({{$precita->id}})"
...
{{-- <form  id="form-deleteee-{{ $precita->id}}"></form>
<button onclick="confirmDelete('form-deleteee-{{ $precita->id}}', '¿Eliminar mascota?', 'Esta acción no se puede deshacer')">
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: '¿Está seguro de eliminar la precita?',
